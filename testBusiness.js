...
function updateOnHand(firstBox, secondBox, thirdBox) {
  ...
  // secondBox and ThirdBox are both cat5, 
  // that means that you can arrive at the
  // cat5 total by combining them

 //  alert ("Values are " + firstBox + " " + secondBox + " " + thirdBox); 

  var catTotal = parseInt(secondBox) + parseInt(thirdBox);


  jobSiteChart.load({
    columns: [
      ['Wire on Hand (Linear Feet)', firstBox, catTotal]
    ]
  });

  // update job table
  document.getElementById("jobCat").innerHTML = catTotal;
  document.getElementById("jobEighteen").innerHTML = firstBox;


  var networkCatTotal = catTotal + 400;
  var networkEighteenTotal = parseInt(firstBox) + 350;

  networkChart.load({
    columns: [
      ['Wire on Hand (Linear Feet)', networkEighteenTotal, networkCatTotal]
    ]
  });
  ...
  document.getElementById("networkCat").innerHTML = networkCatTotal;
  document.getElementById("networkEighteen").innerHTML = networkEighteenTotal;
} 
...
function newUpdate(firstBox, secondBox, thirdBox) {
  ...
  firstOrange = (650 - firstBox) * 5;
  secondOrange = 2000 - secondBox - thirdBox;
...
//	newUpdate(boxOne, boxTwo, boxThree);
        updateOnHand(boxOne, boxTwo, boxThree);
    //  alert("Yes");
      }
    };
